...
func (tileset *DB) GetRowForExport() (*sql.Rows, error) {
	rows, err := tileset.db.Query("select * from tiles")
	if err != nil {
		return nil, err
	}
	return rows, nil
...
func (tileset *DB) GetNext(rows *sql.Rows) ( int, int, int, []byte, error) {
	...
	// get a row
	if rows.Next() {
		err := rows.Scan(&zoom_level, &tile_column, &tile_row, &tile_data)
		if err != nil {
			fmt.Println(err.Error())
			return -1, -1, -1, nil, err
		}
		return zoom_level, tile_column, tile_row, tile_data, nil
	} else {
		return -1, -1, -1, nil, nil
	}
}

func (tileset *DB) CloseRows(rows *sql.Rows) error {
	return rows.Close()
}
