...
//func (itr *Iterator) seekIntegerKey(manifest *Manifest, key string) error {
//	if len(manifest.Entries) > 0 {
//
//		// first go through all the leaf entries and see if it matches
//		for i, entry := range manifest.Entries {
//			// even if the entry is not found, add the pointer to seek so that
//			// seek can continue from the next element
//			if len(entry.Name) > 0 {
...
//		}
//
//		// next go through all the branches to see if any of that matches
//		for i, entry := range manifest.Entries {
//			if entry.EType == IntermediateEntry && (strings.HasPrefix(key, entry.Name)  || strings.HasPrefix(entry.Name, key) ){
//				// found a branch, push the current manifest state
//				manifestState := &ManifestState{
//					currentManifest: manifest,
//					currentIndex:    i + 1,
...
//
//				// now load the child manifest and re-seek
//				childManifest, err := itr.index.loadManifestAndSort(manifest.Name + entry.Name)
//				if err != nil {
//					return err
//				}
//
//				childKey := strings.TrimPrefix(key, entry.Name)
//				return itr.seekIntegerKey(childManifest, childKey)
//			}
//		}
//	}
//	return ErrEntryNotFound
//}
//
//func (itr *Iterator) nextIntegerKey() bool {
//
//	// dont go beyond the limit
//	if itr.limit >= 0 {
//		if itr.givenUntilNow >= itr.limit {
...
//	// check if the search has reached the end key
//	if itr.endPrefix != "" {
//		a, err := strconv.ParseInt(actualKey, 10, 64)
//		if err != nil {
//			return false
//		}
//		b, err := strconv.ParseInt(itr.endPrefix, 10, 64)
//		if err != nil {
//			return false
//		}
//		if a > b {
...
//
//	// if it is a leaf entry, set the key and value
//	if entry.EType == LeafEntry && len(entry.Name) == itr.currentDigits {
//		itr.currentKey = actualKey
//		itr.currentValue = entry.Ref
//		itr.givenUntilNow++
//		return true
...
//	// if it is an intermediate entry, get the branch manifest and push in to the stack
//	if entry.EType == IntermediateEntry {
//		newManifest, err := itr.index.loadManifestAndSort(manifestState.currentManifest.Name + entry.Name)
//		if err != nil {
//			itr.error = err
//			return false
//		}
//
//		// add to the from of the stack
//		newManifestState := &ManifestState{
//			currentManifest: newManifest,
//			currentIndex:    0,
//		}
//		var newStack []*ManifestState
//		newStack = append(newStack, newManifestState)
//		itr.manifestStack = append(newStack, itr.manifestStack...)
//
//		if newManifest.Entries[0].Name == "" {
//			itr.currentKey = actualKey
//			itr.currentValue = newManifest.Entries[0].Ref
//			itr.givenUntilNow++
//			newManifestState.currentIndex = 1 // since we already consumed the first entry
//			return true
//		}
//	}
//	return itr.nextIntegerKey()
//}
//
////func (itr *Iterator) nextIntegerKey() bool {
////	// dont go beyond the limit
