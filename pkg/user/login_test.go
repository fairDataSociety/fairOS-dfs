...
func TestLogin(t *testing.T) {
	...
	t.Run("new-user-multi-cred", func(t *testing.T) {
		...
		login1, _, _, err := userObject.LoginUserV2(user1, "password1", mockClient, "")
		if err != nil {
			t.Fatal(err)
		}

		login2, _, _, err := userObject.LoginUserV2(user1, "password2", mockClient, "")
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("new-user-multi-cred-with-pods", func(t *testing.T) {
		...
		_, _, _, _, ui2, err := userObject.CreateNewUserV2(user1, "password2", mnemonic, "")
		if err != nil {
			t.Fatal(err)
		}
